...
@php
    ...
    $metaTitle = $title ?? $brandName . ' | Alquiler de Movilidades en Cajamarca';
    $metaDescription =
        $description ??
        'Empresa líder en alquiler de movilidades en Cajamarca, traslados seguros, viajes corporativos y transporte turístico hacia los mejores lugares del Perú. Vehículos modernos y conductores profesionales.';
    ...
    $ubicacionesGlobales = \App\Models\Ubicacion::orderBy('nombre')->get();
@endphp
<!DOCTYPE html>
...
<body class="bg-slate-50 text-slate-900 font-sans antialiased flex flex-col min-h-screen">
    ...
    <header class="sticky top-0 z-50 w-full bg-white/95 backdrop-blur-md border-b border-brand-blue/10 shadow-sm"
        role="banner">

        <div
            class="bg-brand-blue text-white py-2 text-[0.75rem] shadow-[0_1px_3px_rgba(0,0,0,0.1)_z-10_relative] w-full">
        ...
        <div
            ...
            <a href="{{ route('inicio') }}" class="shrink-0 flex items-center py-1"
                aria-label="{{ $brandName }} — Inicio">
                <img src="{{ asset('img/logo.webp') }}"
                    alt="{{ $brandName }} — Alquiler de movilidades en Cajamarca y Perú"
                    title="{{ $brandName }} — Alquiler de movilidades en Cajamarca y Perú" width="299"
                    height="300"
                    class="h-12 md:h-14 w-auto object-contain object-left hover:opacity-90 transition-opacity">
            </a>

            <nav class="hidden md:flex items-center justify-center flex-1 gap-2 lg:gap-6 menu-text text-brand-blue"
                aria-label="Navegación principal">

                <div class="relative group">
                    <button type="button"
                        class="group flex flex-col items-center justify-center gap-1.5 focus:outline-none px-2">
                        <img src="{{ asset('iconos/destinos-nacionales.svg') }}"
                            class="w-6 h-6 sm:w-7 sm:h-7 {{ request()->has('ubicacion') ? 'opacity-100 scale-110 drop-shadow-sm' : 'opacity-70 group-hover:opacity-100 group-hover:scale-110' }} transition-all duration-300 object-contain"
                            alt="Destinos Nacionales">
                        <span
                            class="flex items-center gap-1 text-[0.625rem] md:text-[0.7rem] lg:text-[0.8rem] font-bold tracking-wide transition-all text-brand-blue capitalize">
                            Destinos Nacionales
                            <x-dynamic-component :component="'lucide-chevron-down'"
                                class="w-3 h-3 opacity-60 transition-transform group-hover:rotate-180"
                                stroke-width="2.5" />
                        </span>
                    </button>
                    <div
                        class="absolute left-1/2 -translate-x-1/2 top-full mt-2 w-52 bg-white/95 backdrop-blur-md border border-brand-blue/10 shadow-xl rounded-xl flex flex-col py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                        @forelse($ubicacionesGlobales as $ubicacion)
                            <a href="{{ route('flota', ['ubicacion' => $ubicacion->id]) }}"
                                class="px-5 py-2.5 hover:bg-slate-50 hover:text-brand-blue text-xs lg:text-sm text-brand-blue transition-colors font-semibold border-l-2 border-transparent hover:border-brand-yellow capitalize">
                                {{ $ubicacion->nombre }}
                            </a>
                        @empty
                            <span class="px-5 py-2 text-xs text-slate-500 italic">No hay ubicaciones</span>
                        @endforelse
                    </div>
                </div>

                <a href="{{ route('flota') }}"
                    class="group flex flex-col items-center justify-center gap-1.5 focus:outline-none px-2 {{ request()->routeIs('flota') && !request()->has('categoria') && !request()->has('ubicacion') ? '' : '' }}">
                    <img src="{{ asset('iconos/movilidades.svg') }}"
                        class="w-6 h-6 sm:w-7 sm:h-7 {{ request()->routeIs('flota') && !request()->has('categoria') && !request()->has('ubicacion') ? 'opacity-100 scale-110 drop-shadow-sm' : 'opacity-70 group-hover:opacity-100 group-hover:scale-110' }} transition-all duration-300 object-contain"
                        alt="Movilidades">
                    <span
                        class="text-[0.625rem] md:text-[0.7rem] lg:text-[0.8rem] font-bold tracking-wide transition-all text-brand-blue capitalize">Movilidades</span>
                </a>

                <a href="{{ route('flota', ['categoria' => 'Viajes Escolares']) }}"
                    class="group flex flex-col items-center justify-center gap-1.5 focus:outline-none px-2">
                    <img src="{{ asset('iconos/viajes-escolares.svg') }}"
                        class="w-6 h-6 sm:w-7 sm:h-7 {{ request('categoria') === 'Viajes Escolares' ? 'opacity-100 scale-110 drop-shadow-sm' : 'opacity-70 group-hover:opacity-100 group-hover:scale-110' }} transition-all duration-300 object-contain"
                        alt="Viajes escolares" title="Viajes escolares">
                    <span
                        class="flex items-center gap-1 text-[0.625rem] md:text-[0.7rem] lg:text-[0.8rem] font-bold tracking-wide transition-all text-brand-blue capitalize text-center">
                        Viajes Escolares
                    </span>
                </a>
                ...
                <a href="https://adventur.pe/blog" target="_blank" rel="noopener noreferrer"
                    class="group flex flex-col items-center justify-center gap-1.5 focus:outline-none px-2">
                    <img src="{{ asset('iconos/blog.svg') }}"
                        class="w-6 h-6 sm:w-7 sm:h-7 opacity-70 group-hover:opacity-100 group-hover:scale-110 transition-all duration-300 object-contain"
                        alt="Blog" title="Blog">
                    <span
                        class="text-[0.625rem] md:text-[0.7rem] lg:text-[0.8rem] font-bold tracking-wide text-brand-blue transition-all capitalize">Blog</span>
                </a>

                <a href="{{ route('contacto') }}"
                    class="group flex flex-col items-center justify-center gap-1.5 focus:outline-none px-2 {{ request()->routeIs('contacto') ? 'opacity-100 scale-110' : '' }}">
                    <img src="{{ asset('iconos/contacto.svg') }}"
                        class="w-6 h-6 sm:w-7 sm:h-7 {{ request()->routeIs('contacto') ? 'opacity-100' : 'opacity-70 group-hover:opacity-100 group-hover:scale-110' }} transition-all duration-300 object-contain"
                        alt="Contacto">
                    <span
                        class="text-[0.625rem] md:text-[0.7rem] lg:text-[0.8rem] font-bold tracking-wide text-brand-blue transition-all capitalize">Contacto</span>
                </a>
            ...
            <div class="flex items-center gap-2 ml-auto">
                ...
                <div class="hidden md:flex items-center shrink-0">
                    <a href="{{ $whatsappReservaUrl !== '#' ? $whatsappReservaUrl : route('flota') }}"
                ...
                <button type="button"
                    class="md:hidden text-brand-blue hover:bg-slate-100 hover:text-brand-blue transition-colors p-2 rounded-lg -mr-1"
        ...
        <div id="mobile-menu"
            ...
            <div class="max-w-7xl mx-auto px-4 py-4 flex flex-col gap-0.5 menu-text text-brand-blue">
                ...
                <a href="{{ route('flota') }}"
                    class="flex items-center gap-3 px-3 py-3 rounded-xl transition-colors {{ request()->routeIs('flota') && !request()->has('categoria') && !request()->has('ubicacion') ? 'bg-brand-blue/10 text-brand-blue font-bold' : 'hover:bg-slate-100 text-slate-700 hover:text-brand-blue' }}"
                    onclick="toggleMobileMenu()">
                    <img src="{{ asset('iconos/movilidades.svg') }}" class="w-6 h-6 object-contain"
                        alt="Movilidades" title="Movilidades">
                    <span class="text-sm font-bold capitalize tracking-wide text-brand-blue">Movilidades</span>
                </a>

                <details class="group flex flex-col">
                    <summary
                        class="flex items-center justify-between px-3 py-3 rounded-xl text-slate-700 cursor-pointer list-none [&::-webkit-details-marker]:hidden {{ request()->has('ubicacion') ? 'bg-brand-blue/5' : 'hover:bg-slate-100' }}">
                        <div class="flex items-center gap-3">
                            <img src="{{ asset('iconos/destinos-nacionales.svg') }}" class="w-6 h-6 object-contain"
                                alt="Destinos Nacionales" title="Destinos Nacionales">
                            <span class="text-sm font-bold capitalize tracking-wide text-brand-blue">Destinos
                                Nacionales</span>
                        </div>
                        <x-dynamic-component :component="'lucide-chevron-down'"
                    ...
                    <div class="pl-12 pr-3 py-1 flex flex-col gap-1 border-l-2 border-brand-blue/10 ml-6 mb-2 mt-1">
                        ...
                        @empty
                            <span class="py-2 text-xs text-slate-400 italic">No hay ubicaciones</span>
                        @endforelse
                    </div>
                </details>

                <a href="{{ route('flota', ['categoria' => 'Viajes Escolares']) }}"
                    class="flex items-center gap-3 px-3 py-3 rounded-xl transition-colors {{ request('categoria') === 'Viajes Escolares' ? 'bg-brand-blue/10 text-brand-blue font-bold' : 'hover:bg-slate-100 text-slate-700 hover:text-brand-blue' }}"
                    onclick="toggleMobileMenu()">
                    <img src="{{ asset('iconos/viajes-escolares.svg') }}" class="w-6 h-6 object-contain"
                        alt="Viajes Escolares" title="Viajes Escolares">
                    <span class="text-sm font-bold capitalize tracking-wide text-brand-blue">Viajes Escolares</span>
                </a>
    ...
    <main class="grow w-full" id="contenido-principal">
        {{ $slot }}
    </main>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-10 pb-10 border-b border-slate-200">
                ...
                <nav aria-label="Mapa del sitio">
                    ...
                    <ul class="space-y-3 text-base">
                        ...
                        <li><a href="{{ route('flota') }}"
                                class="text-slate-700 hover:text-brand-blue transition-colors {{ request()->routeIs('flota') ? 'text-brand-yellow font-semibold' : '' }}">Flota
                                de Vehículos</a></li>
                        <li><a href="#servicios"
